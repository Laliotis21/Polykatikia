...
Seed προ-οριστικοποιεί **όλο το 2026** (όλα τα κτίρια), σημαδεύει **Α1 (Μαρία) Aug ως Paid**, και για **Κολωνάκι** θέτει `heatingAllocation = METER_READINGS` + ενδείξεις θέρμανσης στους μήνες θέρμανσης (Ιαν demo: 5/20/50/10). Δεν χρειάζεται κλικ Οριστικοποίηση για άδειες οθόνες.
...
2. **Κτίριο:** Κολωνάκι 12 → `/buildings` — «Θέρμανση: μετρητές / ενδείξεις». Άλλα κτίρια: σταθερά χιλιοστά.
3. **Χιλιοστά:** `/buildings/seed-building-kolonaki/shares` — 4 διαμερίσματα με shares.
4. **Ενδείξεις (θέρμανση):** nav «Ενδείξεις» μόνο για Κολωνάκι → `/buildings/seed-building-kolonaki/meters` → μήνας **01/2026** — Α1=5, Α2=20, Β1=50, Β2=10. Badge «Κατανομή: ενδείξεις».
5. **Κοινόχρηστα (μετρητές):** `/buildings/seed-building-kolonaki/koinoxrista` → μήνας **01/2026** — FINALIZED· γραμμές **Θέρμανση** κατά βάρη κατανάλωσης. Badge «Θέρμανση: ενδείξεις».
6. **Κοινόχρηστα (σταθερά shares):** μήνας **08/2026** — χωρίς θέρμανση (καλοκαίρι)· γενικά/ανελκυστήρας από χιλιοστά. FINALIZED. (Παγκράτι κ.λπ. πάντα FIXED_SHARES — χωρίς nav Ενδείξεις.)
...
- `npm run db:seed` upserts owners/portalTokens, categories, monthly expenses, Kolonaki `heatingAllocation=METER_READINGS` + heating-month meter readings, finalizes all months, pays Α1 Aug.
